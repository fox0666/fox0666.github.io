...
!function () {
  ...
  class s {
    ...
    async loadRandModel() {
      ...
      if (this.useCDN) {
        ...
        loadlive2d("live2d", `${this.cdnPath}model/${s}/index.json`), o(["我的新衣服好看嘛？", "换身衣服，心情都变好了呢♪", "嘿嘿，这件新衣服是不是超有气质~", "突然换装，有没有被我吓到呀(✧ω✧)", "这套穿搭，可是精心为你准备的哦~"], 4e3, 10)
      } else fetch(`${this.apiPath}rand_textures/?id=${t}-${s}`).then((e => e.json())).then((e => {
        1!== e.textures.id || 1!== s && 0!== s ? this.loadModel(t, e.textures.id, ["我的新衣服好看嘛？", "换身衣服，心情都变好了呢♪", "嘿嘿，这件新衣服是不是超有气质~", "突然换装，有没有被我吓到呀(✧ω✧)", "这套穿搭，可是精心为你准备的哦~"]) : o(["我还没有其他衣服呢！", "目前只有这一身行头啦，下次给你看新款式~", "衣柜空空如也，快帮我收集更多漂亮衣服吧！"], 4e3, 10)
      }))
    }
  ...
  const n = {
    hitokoto: {
      ...
      callback: function () {
        fetch("https://v1.hitokoto.cn").then((e => e.json())).then((e => {
          const firstMessages = [e.hitokoto, "哇，这句话好有感觉！", "嗯~ 这句话说到我心坎里了呢", "这个句子超有味道，快记下来吧！"];
          const secondMessages = ["这句话的来源好神秘~", "不知道这句话背后藏着什么故事呢？", "悄悄说，我觉得这句话超有灵魂！", "嗯...这句话的出处一定很有趣~", "好想知道是谁想出了这么棒的句子呀！"];
          o(firstMessages, 6e3, 9);
          setTimeout((() => {
            o([secondMessages[Math.floor(Math.random() * secondMessages.length)]], 4e3, 9)
          }), 6e3)
        }))
      }
    },
    ...
    photo: {
      ...
      callback: () => {
        o(["照好了嘛，是不是很可爱呢？", "咔嚓~ 这张照片要好好保存哦！", "镜头感满分！快给我看看拍得怎么样~", "哎呀，别拍啦，人家会害羞的(✿◡‿◡)", "准备好啦，随时可以按下快门哦~"], 6e3, 9);
    ...
    function c(t) {
      ...
      setInterval((() => {
        n ? (n = !1, clearInterval(s), s = null) : s || (s = setInterval((() => {
          o([i, "今天也要元气满满哦！", "加油呀，你是最棒的~", "累了就休息一下，我在这里陪着你~", "不管遇到什么，都要保持微笑哦~"], 6e3, 9)
        }), 2e4))
      }), 1e3);
      o(function (e) {
        ...
        if ("" !== document.referrer) {
          ...
          return location.hostname === e.hostname ? t : (o = s in n ? n[s] : e.hostname, `Hello！来自 <span>${o}</span> 的朋友<br>${["今天天气不错，适合好好学习哦~", "欢迎来到这个小世界，一起玩耍吧！", "初次见面，我是你的看板娘，请多关照~", "很高兴见到你，希望你在这里过得开心~"][Math.floor(Math.random() * 4)]}`)
        }
        return ["欢迎回来！今天也有好好努力嘛~", "好久不见，有没有想我呀？", "欢迎来到这里，让我们一起开启新的旅程~"][Math.floor(Math.random() * 3)]
      }(t.time), 7e3, 11);
      ...
      t.seasons.forEach((({ date: t, text: o }) => {
        ...
        if (n.split("/")[0] <= s.getMonth() + 1 && s.getMonth() + 1 <= c.split("/")[0] && n.split("/")[1] <= s.getDate() && s.getDate() <= c.split("/")[1] && (o = (o = e(o)).replace("{year}", s.getFullYear()), i.push(o)))
      }));
      const a = () => { };
