 ...
 */ !(function () {
    ...
    class s {
        ...
        async loadRandModel() {
            ...
            if (this.useCDN) {
                ...
                loadlive2d('live2d', `${this.cdnPath}model/${s}/index.json`), o('新衣服有没有让我更可爱呀～', 4e3, 10);
            } else
                fetch(`${this.apiPath}rand_textures/?id=${t}-${s}`)
                   .then(e => e.json())
                   .then(e => {
                        1 !== e.textures.id || (1 !== s && 0 !== s) ? this.loadModel(t, e.textures.id, '新衣服有没有让我更可爱呀～') : o('目前只有这一套衣服啦，下次给我换套新的吧～', 4e3, 10);
                    });
        }
    ...
    const n = {
        hitokoto: {
            ...
            callback: function () {
                fetch('https://v1.hitokoto.cn')
                   .then(e => e.json())
                   .then(e => {
                        const t = `哇哦！这句话可是来自 <span>「${e.from}」</span> 呢，是超棒的 <span>${e.creator}</span> 在 hitokoto.cn 分享哒～`;
                        o(e.hitokoto, 6e3, 9),
                            setTimeout(() => {
                                o(t, 4e3, 9);
                            }, 6e3);
        ...
        photo: {
            ...
            callback: () => {
                o('咔嚓！这张照片一定超萌，快保存下来吧～', 6e3, 9), (Live2D.captureName = 'photo.png'), (Live2D.captureFrame = !0);
            }
        },
        info: {
